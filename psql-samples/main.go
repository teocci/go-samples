...
const jinanTestConfig = "user=jinan password=jinan#db host=localhost port=5432 dbname=test_db sslmode=disable pool_max_conns=10"
...
func main() {
	connPoolConfig, err := pgxpool.ParseConfig(jinanTestConfig)
	if err != nil {
	...
	http.HandleFunc("/people/pg", func(w http.ResponseWriter, req *http.Request) {
		...
		_, err := pgStmt.QueryOne(gopg.Scan(&json), rand.Int31n(10000))
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			return
